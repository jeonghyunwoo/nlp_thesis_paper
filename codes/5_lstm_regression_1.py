...
#%% 데이터 가공 및 PCA fittng
df = pd.read_csv('data/lstm_data4.csv',parse_dates=['date'])    
...
#(x_tr, y_tr),(x_te, y_te),(sc_x, sc_y) = make_trte(lookback=1,fcols=topic_sents,fl_name='',target='vix1')
#%% test1
...
perfs.to_csv('data/vix1_perfs.csv')    


#%% test2
fl = ['vix','topic_sents_v','total_sents_v','d2v_v','d2vpca_v','tx1_v','tx2_v']
lkbk = []
feats = []
rmse = []
r2 = []
s = time.time()
for i,feat in enumerate([['vix'],topic_sents,total_sents,d2v,d2vpca,tx1,tx2]):
    for j in [1,3,5,7,14]:
        if i>0:
            fcols = feat+['vix']
        else:
...
#'28:41'
#%%
perfs2 = dict(feature=feats,
             target='vix1',
             lookback=lkbk,
             rmse=rmse,
             r2 = r2)
perfs2 = pd.DataFrame(perfs2)
perfs2
sns.boxplot('feature','rmse',data=perfs2)
perfs2.to_csv('data/vix1_perfs2.csv')    


#%% test3 - spx1 target
fl = ['spx','topic_sents','total_sents','d2v','d2vpca','tx1','tx2','topic_sents_v','total_sents_v','d2v_v','d2vpca_v','tx1_v','tx2_v']
lkbk = []
feats = []
rmse = []
r2 = []
s = time.time()
for i,feat in enumerate([['spx'],topic_sents,total_sents,d2v,d2vpca,tx1,tx2,
                         topic_sents+['spx'],
                         total_sents+['spx'],
                         d2v+['spx'],
                         d2vpca+['spx'],
...
perfs3.to_csv('data/spx1_perfs.csv')            
...
#'02:21:06'
#%%
perfs4 = dict(feature=feats,
             target='spx7',
             lookback=lkbk,
             rmse=rmse,
             r2 = r2)
perfs4 = pd.DataFrame(perfs4)
perfs4
fig,ax = plt.subplots(figsize=(10,4))
sns.boxplot('feature','rmse',data=perfs3,ax=ax)
perfs4.to_csv('data/spx7_perfs.csv')    

#%% test5 - vix7 target
fl = ['vix','topic_sents','total_sents','d2v','d2vpca','tx1','tx2','topic_sents_v','total_sents_v','d2v_v','d2vpca_v','tx1_v','tx2_v']
lkbk = []
feats = []
rmse = []
r2 = []
...
#'02:21:06'
#%%
perfs5 = dict(feature=feats,
             target='vix7',
             lookback=lkbk,
             rmse=rmse,
             r2 = r2)
perfs5 = pd.DataFrame(perfs5)
perfs5
fig,ax = plt.subplots(figsize=(10,4))
sns.boxplot('feature','rmse',data=perfs3,ax=ax)
perfs5.to_csv('data/vix7_perfs.csv')  
gc.collect()